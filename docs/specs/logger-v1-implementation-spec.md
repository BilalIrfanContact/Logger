# Logger V1 implementation specification
...
Logger should let a developer record what happened in their own words, preserve those raw notes, and make the information easy to revisit by journal day or project. AI should help organize the notes, but the user must remain the final authority over saved journal data.
...
3. Logger automatically organizes the day's notes at local midnight. Adding notes to an older day triggers organization immediately.
...
6. Logger stores saved entries under a journal day and an optional project.
...
3. As a developer, I want to sign in with Google, so that I can access Logger without creating another password.
4. As a developer, I want to sign in with GitHub, so that Logger fits my developer workflow.
...
8. As a user, I want to delete my account manually, so that I can remove my Logger data when I choose.
...
15. As a user, I want future dates to remain unavailable for journaling, so that Logger records work that has happened rather than future plans.
...
17. As a user, I want Logger to organize the current day's notes automatically at local midnight, so that I do not have to remember to process them.
...
24. As a user, I want AI to leave uncertain work uncategorized or mark it for review, so that Logger does not invent a project.
...
48. As a user, I want projects to be the V1 categorization system, so that Logger stays simpler than a task-management tool.
...
51. As a user, I want today selected when the calendar opens, so that Logger starts at the most relevant date.
...
68. As a new user, I want Logger to detect an initial locale from my browser or device, so that date formatting works without setup work.
69. As a new user, I want Logger to detect an initial timezone from my browser or device, so that midnight follows my local day.
...
- Download immediately with a filename like `logger-export-2026-08-29.json`.
